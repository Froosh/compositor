...
uint8_t r, g, b;
	...
	// OK, first line is some flags unimplemented at present
	/*snprintf(&message[(ln++ * LINE_SIZE)], LINE_SIZE, "0x%06x\n", 0L);

	// And the next line is the name of the app sending data
	snprintf(&message[(ln++ * LINE_SIZE)], LINE_SIZE, "RAINBOWC\n");
	...
	for (j = 0, k = 0; j < NUM_GLOBES; j++) {
		...
		g = txbuf[k++];
		b = txbuf[k++];
		snprintf(&message[(ln++ * LINE_SIZE)], LINE_SIZE, "0x%02x%02x%02x\n", r, g, b);
	}*/
